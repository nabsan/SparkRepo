...
import java.io.File
  ...
  def main(args: Array[String]): Unit = {
    ...
    val sparkConf = new SparkConf().setAppName("CSV reading in the Riak bucket.")
    ...
    val queryRDD = sc.riakBucket[String](new Namespace("documents")).queryBucketKeys("products")
    //queryRDD.collect().foreach(println)
    println("rdd's count is "+ queryRDD.count())
//    
...
//    .load(queryRDD)
    ...
    val aaa =queryRDD.map{a => 
      val bbb=a.split("¥n")
      (bbb.size)
    }
    println(aaa.foreach(println))
